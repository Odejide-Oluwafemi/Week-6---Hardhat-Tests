...
describe("All Tests", function() {
  ...
  describe("SaveToken", function() {
    ...
    it("Should deposit token by User", async function() {
      ...
      const userInitialTokenBalance = await token.balanceOf(otherAccount.address);
      expect(userInitialTokenBalance).to.be.equal(tokenQuantityBought);

      // User Approves contract to spend token
      const depositAmount = 1;
      await(token.connect(otherAccount).approve(await contract.getAddress(), depositAmount));
      expect(await token.allowance(otherAccount.address, await contract.getAddress())).to.equal(depositAmount); // Check Allowance is set properly
          
      // User can now deposit in our Contract
      const contractInitialTokenBalance = await contract.checkContractTokenBalance(); // Contracts' token balance before user token deposit
      await contract.connect(otherAccount).depositToken(depositAmount);
      
      // Checks
      const contractFinalTokenBalance = await contract.checkContractTokenBalance();
      const userFinalTokenBalance = (await token.connect(otherAccount).balanceOf(otherAccount.address));
      
      expect(userFinalTokenBalance).to.be.lessThan(userInitialTokenBalance);  // that is, User now has a reduced balance
      expect(contractFinalTokenBalance).to.be.greaterThan(contractInitialTokenBalance) // contract now has more balance

      expect(await contract.connect(otherAccount).getUserDepositedTokenBalance()).to.equal(depositAmount);
    });

    it("Should withdraw token by user", async function() {
      const { token, contract, otherAccount } = await loadFixture(deployContracts);
      ...
      await contract.connect(otherAccount).depositToken(depositAmount);
      const userTokenBalanceAfterDeposit = (await token.connect(otherAccount).balanceOf(otherAccount.address));
      ...
      // Checks
      const userFinalTokenBalance = (await token.connect(otherAccount).balanceOf(otherAccount.address));
      expect(userFinalTokenBalance).to.equals(userTokenBalanceAfterDeposit + depositAmount);
    });
  ...
  async function deployContracts() {
    ...
    const MyERC20 = await hre.ethers.getContractFactory("MyERC20");
    const SchoolManagement = await hre.ethers.getContractFactory("SchoolManagementSystem");
  
    const token = await MyERC20.deploy(NAME, SYMBOL, DECIMALS, TOTAL_SUPPLY);
    const contract = await SchoolManagement.deploy(await token.getAddress());

    return { token, contract, owner, user };
  }

  describe("Tests", async function() {
    const { contract, token } = await loadFixture(deployContracts);
    it("Should check if the token address is correctly set", async function() {
      expect(await contract.getTokenAddress()).to.equals(await token.getAddress());
    });
  });
});
  });
